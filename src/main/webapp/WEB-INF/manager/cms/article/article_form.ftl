<@ms.html5>
	...
	<@ms.panel>
		<@ms.form isvalidation=true name="articleForm" action="${managerPath}/cms/article/${action}.do">
            ...
            <@ms.date id="articleDateTime" name="articleDateTime" time=true label="发布时间" single=true readonly="readonly" width="300" value="${(article.basicDateTime?default(.now))?string('YYYY-MM-dd HH:mm:ss')}" validation={"required":"true", "data-bv-notempty-message":"必填项目"} placeholder="点击该框选择时间段"  />
...
$(function(){
	...
	$("#saveUpdate").click(function(){
		...
		var articleDateTimeValue = $("#articleDateTime").val();//让时间能精确到秒与后台对应
